...
const getStudentExamResults = asyncHandler(async (req, res, next) => {
  ...
  // 🔥 Step 1: Resolve studentId from userId (FAST with index)
  ...
  // 🔥 Step 2: Query only relevant exam results
...
const getClassPerformanceAnalysis = asyncHandler(async (req, res, next) => {
  ...
  const results = await ExamResult.find({
    ...
    .populate({
      path: "examination",
      model: Examination,
      select: "examName examType academicYear",
    })
    .populate({
      path: "student",
      model: Student,
...
const generateReportCardPdf = asyncHandler(async (req, res, next) => {
  ...
  if (result.student && result.student.user) {
    ...
    if (typeof u.name === "string") studentName = u.name;
      ...
      studentName = [u.name.firstName, u.name.lastName]
        .filter(Boolean)
        .join(" ");
    }
  }
